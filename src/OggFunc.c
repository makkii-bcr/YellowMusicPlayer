...
int    oggBufLoopCnt_thread = 0; // バッファでループした回数
...
BOOL InitLock(IDirectSoundBuffer8 *pDSBuffer) {
    ...
    // バッファをロックして初期データ書き込み
	// 読み始めは oggSectSize * 25 分だけ書き込んで置く
	...
	//if (IDirectSoundBuffer_Lock(pDSBuffer, oggSectSize * oggFlag, oggSectSize * 5, &AP1, &AB1, &AP2, &AB2, 0) == DS_OK) {
        memset(AP1, 0, AB1);
		getPCMBuffer(&ovf, (char*)AP1, oggSectSize * 5, oggLoopFlag, oggLoopSeek, oggLoopEndSeek, &oggEndFlag);
		IDirectSoundBuffer_Unlock(pDSBuffer, AP1, AB1, AP2, AB2);
		oggFlag = (oggFlag + 5) % oggSect;
	...
	while(!bDone){
		...
		*/
		...
		if (oggPoint2 < oggPoint2_old) {
			if (oggAllWriteFlag_thread < 10) { // 80秒分処理が遅れてしまったら諦める
